...
class SocketIOServerClient:
    ...
    async def on_server_event(self, event: str, handler: Callable):
        ...
        try:
            ...
            async def event_handler(sid, data):
                ...
                # data is likely already a dict if sent as json from client, but purely checking
    ...
    async def wait_for_event(
        ...
        # simplified timeout check logic using asyncio.get_event_loop().time()? 
        # Actually standard time is fine but let's stick to simple logic
        import time
        start = time.time()
